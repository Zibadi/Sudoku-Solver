...
video = cv2.VideoWriter(
    "max-conflict-backtracking.mkv", cv2.VideoWriter_fourcc(*"X264"), 10, (540, 540)
)
...
def find_max_conflict_cell():
    global grid
    conflict_matrix = np.zeros_like(grid)
    for i in range(9):
        for j in range(9):
            if grid[i][j] == 0:
                conflict_matrix[i][j] = count_conflicts(i, j)
    y, x = np.unravel_index(np.argmax(conflict_matrix), conflict_matrix.shape)
    return y, x


def count_conflicts(y, x):
    global grid
    conflicts = []
    for i in range(9):
        conflicts.append(grid[y][i])
    for i in range(9):
        conflicts.append(grid[i][x])
    x0 = (x // 3) * 3
    y0 = (y // 3) * 3
    for i in range(3):
        for j in range(3):
            conflicts.append(grid[y0 + i][x0 + j])
